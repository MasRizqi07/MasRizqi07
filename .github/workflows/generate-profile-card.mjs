...
async function fetchLiveData() {
  ...
  return {
    ...
    contributionRangeStart: fmt(cal.weeks[0].contributionDays[0].date),
    currentStreak: current,
    currentStreakRange: `${fmt(currentStreakRange[0])} - ${fmt(currentStreakRange[1])}`,
    longestStreak: longest,
    longestStreakRange: `${fmt(longestRange[0])} - ${fmt(longestRange[1])}`,
  };
}
...
async function main() {
  ...
  if (!live) console.log("No GITHUB_TOKEN available -- using last-known-real fallback snapshot.");
